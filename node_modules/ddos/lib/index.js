...
const update = function() {
  ...
  if (this.params.testmode && Object.keys(this.table).length > 0) {
    console.log("\n\n\n\n\n\n\nddos: handle: beginning:", this.table);
  }
};
...
const _handle = function(options, table, req) {
  return new Promise((resolve, reject) => {
    ...
    if (!table[host]) {
      ...
      // Increment count and update expiry accordingly
      table[host].count++;
      ...
      if ((table[host].count > options.burst) && (table[host].expiry <= options.maxexpiry)) {
        table[host].expiry = Math.min(options.maxexpiry, table[host].expiry * 2);
      } else {
        table[host].expiry = 1;
      }
    }
    ...
    if (options.testmode) {
      console.log("\n\n\n\n\n\n\nddos: handle: end:", table);
    }

    if (table[host].count > options.limit) {
      if (!options.testmode) {
        console.log("\n\n\n\n\n\n\nddos: denied: entry:", host, table[host]);
      }
      if (options.testmode) {
